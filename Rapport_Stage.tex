...
On peut voir sur l'illustration l'annotation \javacode{@RequestMapping("/dico")} qui précède la déclaration du contrôleur ainsi que l'annotation \javacode{@RequestMapping("/domaines/\{id\}")} avant la déclaration de la méthode qui récupère un domaine selon l'identifiant. 
...
L'outil final pourrait être accessible à de nombreuses personnes au sein de l'entreprise une fois la mise en production. La nécessité de sécuriser la plate-forme est d'abord présente étant donné qu'il est possible de manipuler des données faisant parties intégrantes de la cartographie. Une modification même minime sur les données pourrait entraîner des problèmes, notamment si cela touche des données associées avec d'autres.


Sécurité géré avec Spring Security

On peut voir dans la \autoref{fig:restController} dans le chapitre précédent la présence de l'annotation \javacode{@PreAuthorize("isAuthentificated()")}. Celle-ci permet de refuser l'accès à la méthode si le visiteur ne s'est pas authentifié au préalable. 
...
\begin{figure}
	\includegraphics[scale=0.45]{images/cartoAccueil}
